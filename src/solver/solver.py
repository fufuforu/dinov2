...
class BaseSolver(object):
    ...
    def eval(self): # prepare for evaluation
        self.setup(load_tuning_last_epoch=True)
        self.val_dataloader = dist.warp_loader(self.cfg.val_dataloader, \
            shuffle=self.cfg.val_dataloader.shuffle)
    ...
    def eval_track(self): # prepare for tracking
        self.setup(load_tuning_last_epoch=True)
        self.track_dataset = self.cfg.track_dataset
    ...
    def load_tuning_state(self, path, load_last_epoch=False):
        ...
        if load_last_epoch and 'last_epoch' in state:
            self.last_epoch = state['last_epoch']
