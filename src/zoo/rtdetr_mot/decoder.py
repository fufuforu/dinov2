...
__all__ = ['RTDETRTransformerForMOT']
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        elif attn == 'QuantMultiheadAttention':
            self.self_attn = QuantMultiheadAttention(embed_dim=d_model, num_heads=n_head, dropout=dropout, n_bit=self.n_bit)
        else:
        ...
        if self.extra_track_attn:
            ...
            if attn == 'MultiheadAttention':
                self.update_attn = nn.MultiheadAttention(d_model, n_head, dropout=dropout)
            elif attn == 'QuantMultiheadAttention':
                self.update_attn = QuantMultiheadAttention(d_model, n_head, dropout=dropout, n_bit=self.n_bit)
            else:
                raise NotImplementedError
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        return torch.stack(dec_out_hs), torch.stack(dec_out_bboxes), torch.stack(dec_out_logits)


@register
class RTDETRTransformerForMOT(nn.Module):
    ...
    def forward(self, feats, track_instances: BatchInstances, target_instances: BatchInstances=None):
        ...
        target, init_ref_points_unact, enc_topk_bboxes, enc_topk_logits = \
            self._get_decoder_input(memory, spatial_shapes, denoising_class, denoising_bbox_unact, track_instances=track_instances)

        # decoder
        out_hidden_states, out_bboxes, out_logits = self.decoder(
            target,
