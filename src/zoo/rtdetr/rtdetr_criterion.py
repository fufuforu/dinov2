...
class SetCriterion(nn.Module):
    ...
    def loss_labels_focal(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target = F.one_hot(target_classes, num_classes=self.num_classes+1)[..., :-1].to(src_logits)
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0) # num_match x 4
        ious, _ = box_iou(box_cxcywh_to_xyxy(src_boxes), box_cxcywh_to_xyxy(target_boxes))
        ious = torch.diag(ious).detach() # num_match

        src_logits = outputs['pred_logits'] # B x num_query x C
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)])
        target_classes = torch.full(src_logits.shape[:2], self.num_classes,
                                    dtype=torch.int64, device=src_logits.device)
        target_classes[idx] = target_classes_o # B x num_query
        target = F.one_hot(target_classes, num_classes=self.num_classes + 1)[..., :-1] # B x num_query x C

        target_score_o = torch.zeros_like(target_classes, dtype=src_logits.dtype) # B x num_query
        target_score_o[idx] = ious.to(target_score_o.dtype)
        target_score = target_score_o.unsqueeze(-1) * target # B x num_query x C

        pred_score = F.sigmoid(src_logits).detach() # B x num_query x C
