...
def get_contrastive_denoising_training_group(targets,
    ...
    for i in range(bs):
        num_gt = num_gts[i]
        if num_gt > 0:
            input_query_class[i, :num_gt] = targets[i]['labels'] if isinstance(targets[i], dict) else targets[i].labels
            input_query_bbox[i, :num_gt] = targets[i]['boxes'] if isinstance(targets[i], dict) else targets[i].boxes
