...
from .lsq_plus import LinearLSQ, ActLSQ
# from models.util.quant_base_plus import *
...
class QuantMultiheadAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        self.q_act = ActLSQ(nbits_a=8, in_features=num_heads) if n_bit is not None else nn.Identity()
        self.k_act = ActLSQ(nbits_a=8, in_features=num_heads) if n_bit is not None else nn.Identity()
        self.v_act = ActLSQ(nbits_a=8, in_features=num_heads) if n_bit is not None else nn.Identity()
        self.attn_act = ActLSQ(nbits_a=8, in_features=num_heads) if n_bit is not None else nn.Identity()
